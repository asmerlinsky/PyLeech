...
if __name__ == "__main__":
    ...
    for i in range(num_files):
        for j in range(i, num_files):
            ...
            rt = NLD.getCloseReturns(trace_list[i], trace_list[j], threshold=rt_threshold, get_mask=True)[:, :, 0]
            # plots = NLD.plotCloseReturns(rt, masked=False, reorder=False, get_counts=False, thr=.02)
            ...
            # fig.suptitle(ran_files[i] + "\n" + ran_files[j])
            if i == j:
                # sg_dict = NLD.getCloseReturnsSegmentsFromUnorderedMatrix(rt & ~zeros, zeros, rt_len=25,
                #                                                          single_file=True, min_dist=10)
                sg_dict = NLD.getCloseReturnsSegmentsFromUnorderedMatrix(rt, zeros, rt_len=30,
                                                                         single_file=True, min_dist=10, true_rt_pct=.01)
            else:
                # sg_dict = NLD.getCloseReturnsSegmentsFromUnorderedMatrix(rt & ~zeros, zeros, rt_len=25)
                sg_dict = NLD.getCloseReturnsSegmentsFromUnorderedMatrix(rt , zeros, rt_len=30, true_rt_pct=.01)
    ...
    for file_tup, items1 in segments.items():
        ...
        for diagonal in list(items1):
            for tup0, tup1 in items1[diagonal]:
                ...
                if t0.min() <8 and t0.max()>12:
                        fig1, ax1 = plt.subplots()

                        # r0 = np.arange(t0.shape[0]) - np.argmax(t0)
                        # r1 = np.arange(t1.shape[0]) - np.argmax(t1)
                        ax1.plot( t0, linewidth=1, color='k')
                        ax1.plot( t1, linewidth=1, color='k')
