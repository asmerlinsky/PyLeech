# -*- coding: utf-8 -*-
"""
Created on Thu Aug 30 10:51:45 2018
...
def getIpspResults(filename, spike_amp=[0.3, 0.5], min_spike_per_sec=10, channels=['Vm1', 'IN5'], spike_max_dist=0.7):
    ...
    burst_list = sU.getBursts(indexes, block.get_signal_sampling_rate(), spike_max_dist, min_spike_no=20,
    ...
    for burst_indexes in burst_list:
        ...
        result_dict[trace + '.' + str(i)] = {'burst data': burst_dict, 'T data': T_dict}
        i += 1
    del (block)
    return result_dict
...
if __name__ == "__main__":
    ...
    # try:
    #    full_file_results = json.load(open('results_by_neuron.json'))
    ...
    for neuron, spike_amp in file_list.items():
        full_file_results[neuron] = {}

    for neuron, spike_amp in file_list.items():
        neuron_traces = glob.glob('*/' + neuron + '*.abf')
        for trace in neuron_traces:
            trace_results = getIpspResults(trace, spike_amp)

            full_file_results[neuron].update(trace_results)

    dumped = json.dumps(full_file_results, cls=json_numpy.NumpyAwareJSONEncoder)
    f = open("results_by_neuron.json", "w")
    f.write(dumped)
    f.close()
