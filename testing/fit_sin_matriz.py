...
#        print(grad_Neg_Log_Likelihood[41*scaling_length:41*scaling_length + scaling_length])
        ...
        for n in range(no_neurons):
            conn_grad_Neg_Log_Likelihood = spsig.correlate(padded_spike_matrix[n][::-1], spikes_minus_exp, mode='valid')[1:]
...
def gradPenalizedNegLogLikelihood(fit_array, input_stim, spike_matrix, dt, scaling_length, fit_k_len, full_k_len,
    ...
    for i in range(no_neurons):
        ...
        penalty_grad[1:] -= 2 * penalty_param * np.diff(neuron_filters[i][1:fit_k_len + 1])
        # print(1 + neuron_filter_length * i, 1 + neuron_filter_length * i + fit_k_len)
...
if __name__ == '__main__':
    ...
    mn = srmSimUtils.SRMMultiNeuronSimulator(dt=1/1000, sim_length=60*8, no_sim=1, k_length=1, ref_length=1,
                                             **neurons)
    mn.runSimulation()
    mn.plotSimulation()
    ...
    penfit =  SRMFitter(mn.time_steps, mn.input_stim[0], spike_freq_dict, k_duration=1, refnconn_duration=0.5, dim_red=50)
    ...
    fit1.minimizeNegLogLikelihood(4000, use_grad=True, func_jac_list=[NegLogLikelihood, gradNegLogLikelihood],
                                  verbose=False)
    penfit.minimizeNegLogLikelihood(4000, use_grad=True, func_jac_list=[penalizedNegLogLikelihood, gradPenalizedNegLogLikelihood],
                                  verbose=True, penalty_param=.1)
    ...
    plt.plot(gradNegLogLikelihood(penfit.fit_array, penfit.input_stim, penfit.spike_count, penfit.dt, penfit.scaled_fit_size,
                           penfit.k_length,
                           penfit.k_length * penfit.scaled_fit_size, penfit.single_conn_length,
                           penfit.single_conn_length * penfit.scaled_fit_size, 0), label='gNLLH', linestyle='--')
    plt.plot(
        gradPenalizedNegLogLikelihood(penfit.fit_array, penfit.input_stim, penfit.spike_count, penfit.dt, penfit.scaled_fit_size,
                           penfit.k_length,
                           penfit.k_length * penfit.scaled_fit_size, penfit.single_conn_length,
