...
for neuron, amp in file_list.items():
    for trace in glob.glob('*/' + neuron + '*.abf'):
        ...
        burst_list = sU.getBursts(indexes, block.get_signal_sampling_rate(), spike_max_dist=0.6, min_spike_no=20, min_spike_per_sec=10)
        ...
        if False:
            for i in range(0, len(burst_list)):
                if j == 0:
                    ax[0].plot([time[burst_list[i][0]], time[burst_list[i][-1]]], [baseline_list[i], baseline_list[i]], color='r', label = 'baseline')
                    j += 1
                ax[0].plot([time[burst_list[i][0]], time[burst_list[i][-1]]], [baseline_list[i], baseline_list[i]], color='r')
