...
def peak(x, minimal_dist=15, not_zero=1e-3):
    ...
    return np.array(scipy.signal.find_peaks(x, height=1, distance=minimal_dist)[0])
    """
    Old method
...
def plot_events(evts_matrix,
    ...
    for i in range(n_plot):
        if pltobj is None:
            ...
            pltobj.plot(evts_matrix[i, :], color=events_color, lw=events_lw)
    if show_median:
        MEDIAN = np.apply_along_axis(np.median, 0, evts_matrix)
        if pltobj is None:
            plt.plot(MEDIAN, color=median_color, lw=median_lw)
        else:
            pltobj.plot(MEDIAN, color=median_color, lw=median_lw)
    ...
    if show_mad:
        ...
        if pltobj is None:
            ...
            pltobj.plot(MAD, color=mad_color, lw=mad_lw)
    ...
    if pltobj is None:
        ...
        for l in left_boundary:
            pltobj.axvspan(l, l + cut_length - 1,
                           facecolor='grey', alpha=0.5, edgecolor='none')
    ...
    for i in range(nb_chan):
        ...
        if (channels is None) or (i in channels) or (-1 in channels):
            ...
            if peak_color is not None:
                if not labeled:
                    plt.scatter(time_axes[evts_pos],
                                data_list[i][evts_pos] - display_offset[i], marker='o', color=peak_color, label=label)
                    labeled = True
                else:
                    plt.scatter(time_axes[evts_pos],
                                data_list[i][evts_pos] - display_offset[i], marker='o', color=peak_color)
            else:
                if not labeled:
                    plt.scatter(time_axes[evts_pos],
                                data_list[i][evts_pos] - display_offset[i], marker='o', label=label)
                    labeled = True
                else:
                    plt.scatter(time_axes[evts_pos],
                                data_list[i][evts_pos] - display_offset[i], marker='o')
    plt.yticks([])
    plt.xlabel("Time (s)")
...
def mk_center_dictionary(positions, data, before=49, after=80, dataD=None, dataDD = None):
...
def classify_and_align_evt(evt_pos, data, centers,
    ...
    the name of the closest center if it is close enough or nan, the
    ...
    A list with the following components:
      The name of the closest center if it was close enough or nan.
    ...
    if sum(evt ** 2) > sum((h - jitter1 * centerD - jitter1 ** 2 / 2 * centerDD) ** 2):
        ...
        return [nan, evt_pos, jitter1]


def align_evt(evt_pos, assigned_cluster, data, centers,
              before=14, after=30):
    """Compares a single event to a dictionary of centers and returns
    the name of the closest center if it is close enough or nan, the
    corrected peak position and the remaining jitter.
    ...
    ----------
    evt_pos : a sampling point at which an event was detected.
    assigned_cluster: the cluster to which the peak was assigned
    data : a data matrix.
    centers : a centers' dictionary returned by mk_center_dictionary.
    before : the number of sampling point to consider before the peak.
    after : the number of sampling point to consider after the peak.
    ...
    c_name = assigned_cluster
    centerM = np.array(
        centers[c_name]["center"][
            np.tile((-before <= centers[c_name]["center_idx"]).__and__(centers[c_name]["center_idx"] <= after),
                    n_sites)]
    )
    evt = cut_sgl_evt(evt_pos, data=data, before=before, after=after)
    delta = -(centerM - evt)
    good_cluster_name = assigned_cluster
    ...
    if abs(round(jitter1)) > 0:
        ...
        if h_order0_norm2 > h_order1_norm2:
            ...
            jitter1 = 0
    if sum(evt ** 2) > sum((h - jitter1 * centerD - jitter1 ** 2 / 2 * centerDD) ** 2):
        return [assigned_cluster, evt_pos, jitter1]
    else:
        return [nan, evt_pos, jitter1]
