...
class CrawlingSegmenter():
    ...
    def select_spikes(self, *args):
        ...
        cmap = burstUtils.categorical_cmap(math.ceil(len(self.selected_spikes) / math.ceil(len(self.selected_spikes) / 10)),
...
class SegmentandCorrelate(NSSegmenter):
    ...
    def __init__(self, spike_freq_dict, intracel_signal, time_vector, intracel_fs=20000, time_intervals=None,
                 no_cycles=1, intracel_cutoff_freq=1, intracel_peak_height=None, intracel_peak_distance=10,
                 intracel_prominence=5, outlier_threshold=5, sigma=3, time_range=20, bin_step=0.1):
    ...
    def generateInterval(self, cutoff_freq=1, peak_height=None, peak_distance=10, prominence=5, no_cycles=None):
        ...
        for interval in self.time_intervals:
            ...
            loc = np.where((self.time[::int(self.bin_step * self.fs)]>interval[0]) & (self.time[::int(self.bin_step * self.fs)]<interval[1]))[0]
            ...
            if peak_height is None:
                ...
                # plt.figure()
                # plt.plot(self.filtered_intracel[loc])
                print(peak_distance, peak_height, prominence)
            intracel_interval_idxs = spsig.find_peaks(self.filtered_intracel[loc], height=peak_height, distance=peak_distance, prominence=prominence)[
                0] + loc0

            intracel_max_idxs.append(
                intracel_interval_idxs[(self.time[::int(self.bin_step * self.fs)][intracel_interval_idxs] > interval[0])
                                                   &
                                  (self.time[::int(self.bin_step * self.fs)][intracel_interval_idxs] < interval[1])])


        assert len(np.concatenate(
            intracel_max_idxs)) > 0, 'No peak was found for the intracelular signal and the given time interval'

        self.intervals = []

        for idx_list in intracel_max_idxs:
            for i in range(len(idx_list) - no_cycles):
        ...
        intracel_max_idxs = [idx for subl in intracel_max_idxs for idx in subl]
        plt.figure()

        # plt.plot(self.time[::100], self.intracel_signal[::100])
        first = self.time_intervals[0][0]
        last = self.time_intervals[-1][1]
        idxs = np.where((self.time[::int(self.bin_step * self.fs)]>first) & (self.time[::int(self.bin_step * self.fs)]<last))[0]
        plt.plot(self.time[::int(self.bin_step * self.fs)][idxs], self.filtered_intracel[idxs], color='k')

        plt.scatter(self.time[::int(self.bin_step * self.fs)][intracel_max_idxs],
                    self.filtered_intracel[intracel_max_idxs], color='r', zorder=50)
    ...
    def getSegmentsEmbeddings(self, trace='NS'):
        ...
        if trace == 'NS':
            for segment in self.NS_segments:
                embedding_list.append(NLD.getDerivativeEmbedding(segment, self.bin_step, 3))
        else:
            for segment in self.segmented_neuron_frequency_list:
                embedding_list.append(NLD.getDerivativeEmbedding(segment[trace], self.bin_step, 3))
...
if __name__ == "__main__":
    ...
    segments = SegmentandCorrelate(spike_freq_dict,
                        NS,
                        time,
                        intracel_fs=fs,
                        time_intervals=crawling_interval,
                        no_cycles=1,
                        intracel_cutoff_freq=1,
                        intracel_peak_height=-50,
                        intracel_peak_distance=10,
                        intracel_prominence=5,
                        outlier_threshold=5,
                        sigma=3,
                        time_range=20,
                        bin_step=0.1
